...
4. **动作也用同一份表转** (面板 ③): 点**导入动画**批量导进来 (或点"列出文件里的
   动作"把 blend 里已有的列出来), 勾选要转的, 再点 **转换 →** / **← 还原**。
...
**导入动画** 按钮: 批量导入 `.fbx` / `.blend`, **只留下动作** —— 导进来的骨架、
网格、材质、贴图当场全部回收 (按数据块快照精确回收, 不是事后 purge 孤儿),
文件里只多出动作本身。可多选文件, 或勾选"整个文件夹"把整个目录一次导完;
动作按文件名命名并打 fake user, 导完直接进下面的列表。`.blend` 走库加载,
连对象都不会产生。CLI 的 `--import-fbx` 与它共用同一套底层。
...
只要动作的批量导入(FBX 往返后零对象零骨架残留, 面板与 CLI 共用同一套底层)。
